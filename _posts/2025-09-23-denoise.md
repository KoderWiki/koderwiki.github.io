...
먼저 Non-local Means argorithm이다. <br>
...
이에 반해 Non-local Means는 이름 그대로 local 영역의 pixel이 아닌 해당 pixel 주변 영역과 비슷한 영역을 찾아 평균을 취하는 방식으로 노이즈를 줄이는 기법이다. <br>
...
def run(path, file, scale = 1.0,
    ...
    df = pd.DataFrame(rows).sort_values(by=["PSNR", "SNR_proxy"], ascending=False)
